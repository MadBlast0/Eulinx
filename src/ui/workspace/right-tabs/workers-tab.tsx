...
export function WorkersTab() {
  ...
  return (
    <div className="flex-1 overflow-y-auto pt-2">
      {workers.length === 0 ? (
        <div className="flex flex-col items-center gap-2 px-4 py-8 text-sm text-[color:var(--Eulinx-color-text-muted)]">
          <span>No workers running.</span>
          <span className="text-xs">Spawn a worker to get started.</span>
        </div>
      ) : (
        workers.map((worker) => (
          <div
