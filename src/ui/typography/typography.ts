/**
 * Eulinx Typography System
 *
 * Object model + literal type scale. Canonical, build-time-frozen typography
 * tokens for Eulinx. Implements Typography-Part01 / Typography-Part03.
 *
...
/**
 * The complete Eulinx type scale. Exactly ten roles, frozen at build time.
 * Literal values from Typography-Part03 / task spec.
 *
 * display   30 / 1.25 / -0.01em / 700
 * heading1  24 / 1.30 / -0.01em / 600
 * heading2  20 / 1.35 /  0.00em / 600
 * heading3  18 / 1.40 /  0.00em / 600
 * heading4  16 / 1.45 /  0.00em / 600
 * body      14 / 1.60 /  0.00em / 400
 * label     13 / 1.45 /  0.01em / 500
 * caption   12 / 1.45 /  0.01em / 400
 * code      13 / 1.60 /  0.00em / 400  (mono, tnum on)
 * terminal  13 / 1.55 /  0.00em / 400  (mono, liga+calt OFF)
 */
 ...
 *
 * cellWidthPx = fontSizePx * 0.6  (invariant — advance ratio of the file)
 * cellHeightPx = fontSizePx * lineHeight
 * baselineOffsetPx uses a nominal ascent ratio (0.8 of cell height) which is
 * a safe layout constant for JetBrains Mono; the terminal aligns cells by
 * cellHeight, not by baseline, so this is for caret/glyph placement only.
 */
