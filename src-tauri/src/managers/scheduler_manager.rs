...
#[cfg(test)]
mod tests {
    // All scheduler_manager tests require a Wry AppHandle which cannot be
    // constructed from MockRuntime in Tauri 2.11. These tests are preserved
    // as documentation but gated behind `#[ignore]` so they don't block
    // compilation of the rest of the test suite.
}
